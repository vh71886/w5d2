...
def travoltas_busiest_years
  ...
  # Why doens't COUNT(movies.title) AS movie_count...HAVING movie_count >= 2 work?
  execute(<<-SQL)
    SELECT
      movies.yr, COUNT(*)
    FROM
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    SELECT
      movies.title, COUNT(DISTINCT actors.id) AS cast_count
    FROM
      movies
    JOIN castings ON movies.id = castings.movie_id
    JOIN actors ON actors.id = castings.actor_id
    ...
    GROUP BY
      movies.title
    ORDER BY
      cast_count DESC, movies.title ASC;
  SQL
end
